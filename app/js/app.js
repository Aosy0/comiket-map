...
const App = {
  ...
  bindEvents() {
    ...
    if (closeSyncQR) {
      closeSyncQR.addEventListener('click', () =>
        document.getElementById('syncQRModal').classList.add('hidden'));
    }

    const closeSyncURL = document.getElementById('closeSyncURLModal');
    if (closeSyncURL) {
      closeSyncURL.addEventListener('click', () =>
        document.getElementById('syncURLModal').classList.add('hidden'));
    }

    const closeSyncCode = document.getElementById('closeSyncCodeModal');
    if (closeSyncCode) {
      closeSyncCode.addEventListener('click', () =>
        document.getElementById('syncCodeModal').classList.add('hidden'));
    }
  ...
  showShareQR() {
    ...
    document.getElementById('syncInfo').textContent =
      `${size.circles}件のサークルデータ (${size.humanReadable})`;
  ...
  showShareURL() {
    ...
    document.getElementById('syncURLInfo').textContent =
      `${size.circles}件のサークルデータ (${size.humanReadable})`;
